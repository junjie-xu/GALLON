...
def query(args):
    ...
    for _, mol in enumerate(dataset[args.begin:]):
        ...
        if args.diagram and args.structure:
            json_path = os.path.join('./gpt_response', dataname, str(index) + '.json')
        elif (not args.diagram) and args.structure:
            json_path = os.path.join('./gpt_response', dataname, 'no_diagram', str(index) + '.json')
        elif args.diagram and (not args.structure):
            json_path = os.path.join('./gpt_response', dataname, 'no_structure', str(index) + '.json')
        elif (not args.diagram) and (not args.structure):
            json_path = os.path.join('./gpt_response', dataname, 'text_only', str(index) + '.json')
